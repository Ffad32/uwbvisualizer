...
function toFixed4(num: number): string {
  return num.toFixed(4);
}

// Simulated tag position function (Lissajous)
function tagPosition(t: number): Point {
...
function batteryVoltage(t: number): number {
  return 3.95 + 0.25 * Math.sin(t * 0.02); // period ~314 seconds
}

// POST data to server
function postData(distances: number[], battery: number, chipId: string) {
...
function postAnchorData(anchorNumber: number, battery: number, chipId: string) {
  const postData = JSON.stringify({
    chip_id: chipId,
    role: 'anchor',
